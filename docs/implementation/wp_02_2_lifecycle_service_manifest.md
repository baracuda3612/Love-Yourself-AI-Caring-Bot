...
| Scheduler expiry and former independent ignored-task scan | `expire_plan_step()` | Each overdue candidate runs and commits in its own transaction. Expected pre-/post-catalog chronology and supported legacy/JSON-only linkage gaps return `telemetry_not_recorded` without reopening the step; an unexpected invariant/storage failure rolls back and is logged for only that candidate, while earlier commits survive and later candidates continue. | `remove_step_keyboard` is retried until confirmed; feature-wide telemetry remains WP-07.1. |
...
| Coach day/evening change and `/user/time-slots` | `change_delivery_time()` | Canonicalizes accepted time once, persists the slot and future step times, and records that same value in the receipt/result. Exact replay invokes a plan-wide proof under the same user-row fence used by lifecycle decisions: every eligible future job is exact and every obsolete deterministic job is absent, including a stale job whose new time is already past. A newer time/cancel decision therefore cannot be overwritten by an older scheduler snapshot. Superseded replay is non-success with the authoritative value. | Paused decisions are saved but explicitly `deferred`; semantic time validation/collection remains WP-02.3. API authentication/enrollment remains WP-04.1. |
