...
| Coach follow-up activation | `activate_plan()` | Locks the entitled user, enforces no-current-plan/history rules, and reserves the plan type plus resolved delivery times in the activation receipt. Exact replay returns the recorded activation outcome; argument drift fails closed. | `reconcile_plan_schedule` runs after commit. Target generation remains WP-03.2; deterministic onboarding remains WP-04.3. |
...
| Scheduler expiry and former independent ignored-task scan | `expire_plan_step()` | Expiry and canonical `task_ignored` event share one source operation and transaction. A legacy pre-catalog expiry terminalizes the step without manufacturing a historical event, while per-step savepoints keep another invalid legacy event from aborting the sweep. | `remove_step_keyboard` is retried until confirmed; feature-wide telemetry remains WP-07.1. |
...
| Coach day/evening change and `/user/time-slots` | `change_delivery_time()` | Validates the owned lifecycle context, persists the slot and future step times, records a receipt, and does not overwrite a later value on replay. Callers expose a superseded replay as non-success with the current authoritative value, while exact replay reconstructs only still-future pending scheduler targets. | Active jobs reconcile after commit only for authoritative, schedulable decisions; paused reconciliation and semantic collection UX remain WP-02.3. API authentication/enrollment remains WP-04.1. |
...
| Plan-format request | `request_plan_format_switch()` | Validates the enabled owner, current plan, source ID, and target format; reserves a durable deferred receipt keyed to the target plan/format; and returns `applied=False`. | Actual format-switch behavior belongs to WP-02.3. |
