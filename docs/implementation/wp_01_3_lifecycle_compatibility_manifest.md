...
| exported plan builder/finalizer activation entrance | WP-02.2 routes live activation only through `app.lifecycle.activate_plan()`; the builder/finalizer remains an internal persistence helper with no scheduler or Telegram side effect. | Internal call from `app.lifecycle` only. | WP-03.2 replaces target generation; lifecycle authority remains in `app.lifecycle`. |
| direct pause/step guard modules and time-slot API mutation | WP-02.2 deletes `app.plan_pause` and `app.plan_guards`, and routes callbacks, Coach controls, scheduler transitions, and API time changes through `app.lifecycle`. | None outside the lifecycle boundary; presentation and post-commit reconciliation only. | Detailed control UX is WP-02.3; API access authority is WP-04.1. |
