...
from app.ux.persona import PERSONA_TONE, get_persona, get_sent_indices
...
async def generate_pulse_message(user_profile, llm_client) -> tuple[str, list[int]]:
    """Returns (message_text, updated_sent_indices)."""
    persona = get_persona(user_profile)
    sent = get_sent_indices(user_profile)
    result = get_unused_quote(persona, sent)

    if not result:
        return get_coach_voice(persona) or "", sent
    ...
    new_indices = (sent + [idx])[-50:]
    text = f'<i>"{escape(quote_text)}"</i>\n— {escape(author)}\n\n{escape(commentary)}'
