...
def test_normal_scenario_counts(monkeypatch):
    db = DummyDB(DummyPlan(id=10, total_days=7, focus="ENERGY", load="LITE", duration="2w"))
    delivered = [
    ...
    monkeypatch.setattr(metrics, "_fetch_delivered_steps", lambda *_args, **_kwargs: delivered)
    monkeypatch.setattr(metrics, "get_completion_rate", lambda *_args, **_kwargs: 1 / 3)
...
def test_zero_delivered_returns_weak_defaults(monkeypatch):
    db = DummyDB(DummyPlan(id=20, total_days=5, focus="CALM", load="STANDARD", duration="1w"))

    monkeypatch.setattr(metrics, "_fetch_delivered_steps", lambda *_args, **_kwargs: [])
...
def test_best_streak_calculated_by_consecutive_days(monkeypatch):
    db = DummyDB(DummyPlan(id=30, total_days=10, focus=None, load="LITE", duration=None))
    delivered = [
    ...
    monkeypatch.setattr(metrics, "_fetch_delivered_steps", lambda *_args, **_kwargs: delivered)
    monkeypatch.setattr(metrics, "get_completion_rate", lambda *_args, **_kwargs: 0.75)
...
def test_dominant_time_slot_and_tie(monkeypatch):
    db = DummyDB(DummyPlan(id=40, total_days=3, focus="FOCUS", load="LITE", duration="1w"))
    dominant_delivered = [
    ...
    monkeypatch.setattr(metrics, "_fetch_delivered_steps", lambda *_args, **_kwargs: dominant_delivered)
    monkeypatch.setattr(metrics, "get_completion_rate", lambda *_args, **_kwargs: 1.0)
    monkeypatch.setattr(metrics, "get_adaptation_count", lambda *_args, **_kwargs: 0)
    ...
    monkeypatch.setattr(metrics, "_fetch_delivered_steps", lambda *_args, **_kwargs: tie_delivered)
...
def test_outcome_tier_thresholds(monkeypatch, rate, expected):
    db = DummyDB(DummyPlan(id=50, total_days=3, focus="ENERGY", load="LITE", duration="1w"))
    delivered = [(DummyStep(day=DummyDay(1), is_completed=True, time_slot="MORNING"), _ts(1))]

    monkeypatch.setattr(metrics, "_fetch_delivered_steps", lambda *_args, **_kwargs: delivered)
    monkeypatch.setattr(metrics, "get_completion_rate", lambda *_args, **_kwargs: rate)
    monkeypatch.setattr(metrics, "get_adaptation_count", lambda *_args, **_kwargs: 0)
