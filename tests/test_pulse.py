...
def _make_days(total=21, adapted_day=None):
    days = []
    for i in range(1, total + 1):
        days.append(_Day(day_number=i, steps=[]))
    if adapted_day:
        days[adapted_day - 1].adapted = True
    return days


def test_active_day_excludes_paused():
    plan = SimpleNamespace(id=1, user_id=10, total_days=21, current_day=None)
    user = SimpleNamespace(profile={"persona": "empath"})
    days = [_Day(1, [], status="active"), _Day(2, [], status="paused"), _Day(3, [], status="active")]
    data = build_pulse_data(1, _DB(plan, user, days))
    assert data.active_day_number == 2
...
def test_adapted_past():
    data = build_pulse_data(1, _DB(SimpleNamespace(id=1, user_id=10, total_days=21, current_day=14), SimpleNamespace(profile={}), _make_days(21, adapted_day=1)))
    entry = data.days[0]
    assert entry.adapted is True
    assert entry.window == "past"


def test_adapted_active():
    data = build_pulse_data(1, _DB(SimpleNamespace(id=1, user_id=10, total_days=21, current_day=14), SimpleNamespace(profile={}), _make_days(21, adapted_day=10)))
    entry = data.days[9]
    assert entry.adapted is True
    ...
    class _DBShort:
        def query(self, model):
            ...
            if model is scheduler.User:
                return _Query(SimpleNamespace(id=10, tg_id=100, timezone="Europe/Kyiv"))
